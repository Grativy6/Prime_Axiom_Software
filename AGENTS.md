...
The Build 002 verifier preserves inherited Build 000/001 evidence, requires a zero-skip test pass, executes the pinned HDL matrix, replays result generation twice, and verifies every manifest hash. Use `& .\scripts\verify-build001.ps1` or `& .\scripts\verify.ps1` only for their inherited narrower paths.
